...
function eliminarProducto(event){
    ...
    if(event.target.id === 'botonEliminarProducto'){
        console.dir(event.target.parentElement.firstChild.parentNode.attributes.key.value) //tengo que recuperar el atributo key del li
        //const textoABuscar = event.target.parentElement.firstChild.data.trim(); //recupero el texto a buscar.
        const idABuscar = event.target.parentElement.firstChild.parentNode.attributes.key.value;
...
function pintarProductos(elementos){
    const nuevoArray = elementos.map(
        valor => `<li key=${valor.id}> ${valor.nombre} <button id="botonEliminarProducto">Eliminar</li>`
    );
